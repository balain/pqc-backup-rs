...
`PQBACK02` archive encoding and `PQROOT02` key encoding version 1 are frozen by their specifications and deterministic vectors.
...
The current decoder accepts only `PQBACK02` version 2 and `PQROOT02` key version 1. Unknown magic, version, algorithm ID, or extra header bytes are rejected. Decoders must dispatch on the outer magic/version before interpreting version-specific fields; they must not guess based on file length.

## Backward compatibility

Future releases that claim v2 compatibility must keep reading every valid `PQBACK02` vector and real v2 archive. The deterministic vector test is a mandatory release gate.
